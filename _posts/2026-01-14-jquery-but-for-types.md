...
To encode this diagram in Rust with typestate, each branch of an "OR" is a unique struct, and each branch of "AND" is a type parameter to a struct.
...
```rust
ResultSet<Open<CurrentRow<ValidRow<Read>>, ReadOnly, ForwardOnly>
```
...
```rust
impl<CRowState, Concurrency, Direction> ResultSet<Open<CurrentRow<CRowState>>, Concurrency, Direction> {
    pub fn next(mut self) -> Result<ResultSet<Open<CurrentRow<ValidRow<NotYetRead>>, Concurrency, Direction>>, ResultSet<Open<CurrentRow<InvalidRow>, Concurrency, Direction>>> {
        ...
    }
}
```

Whew, that's lot of characters for a return type! Contrast this with the concision of the ResultSet documentation:
...
RS.SetState.Position.CRowState = InvalidRaw;
```

That kind of nested mutation on a struct works perfectly fine for values, but we don't have an equivalent notation for types. Another way to think about it is a language of selectors. In JavaScript, you can use JQuery to access an arbitrary part of the DOM tree:

```js
$('#result-set .set-state .position .c-row-state').html(`<div class = "InvalidRow"/>`);
```
...
First, we need some way of reading and writing to the sub-pieces of type. For example, `Open<P, C, D>` is essentially a tuple with tree components. I want an API that looks like:

```rust
Get<Open<P, C, D>, 0> = P
Get<Open<P, C, D>, 2> = D
Set<Open<P, C, D>, 1, T> = Open<P, R, D>
```
...
type RS2 = UpdateCurrentRow<ResultSet<Open<CurrentRow<S>, C, D>>, T>;
...
```rust
type UpdateCurrentRow<RS, T> = Replace<RS, (U0, (U0, ())), T>;
```

Now our challenge is to implement the `Replace` operator. It should take as input a type, a selector, and new type to replace at the location of the selector. Before looking at the glory Rust code, consider a simple implementation in OCaml-style:

```rust
let rec replace(t1: type) (sel: int list) (t2: type) = match sel with
| [] -> t2
| (n :: sel') =>
...
To replace a type, we recursively iterate over the selector list. In the base case, we return the replacing type. In the inductive case, we get the N-th type from `t1`, then recurse with the remaining sel`. We place this updated type back into its context in t1 using type_set.
...
pub type Replace<T, Selector, Replacement> = <T as ComputeReplace<Selector, Replacement>>::Output;
```

The base case if fairly simple:
...
where
    ...
    T: ComputeSetType<Sel, Replace<GetType<T, Sel>, SelList, Replacement>> {
        type Output = SetType<T, Sel, Replace<Get<T, Sel>, SelList, Replacement>>;
    }
```

Again, this code makes most sense when placed in correspondence with the OCaml program. The `type Output` section describes the actual computation. The `where` bound is necessary to make the computation possible.
...
```rust
type UpdateCurrenRow<RS, T> = Replace<RS, (U0, (U0, (U0, ()))), T>;

impl<S, C, D> ResultSet<Open<CurrentRow<S>, C, D>> {
    pub fn next(mut self) -> Result<UpdateCurrentRow<Self, ValidRow<NotYetRead>>, UpdateCurrentRow<Self, InvalidRow>> {
        panic!()
    }
}
```
...
Wrapping up, we've seen how to implement a selector language for replacing nested types. The language is still pretty far from actual JQuery. There's no way to tag classes to types, or to distinguish between a direct vs indirect descendent.